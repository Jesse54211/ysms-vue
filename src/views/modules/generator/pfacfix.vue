...
<!--        <el-button v-if="isAuth('generator:pfacfix:save')" type="primary" @click="addOrUpdateHandle()">新增</el-button>-->
<!--        <el-button v-if="isAuth('generator:pfacfix:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>-->
      </el-form-item>
    </el-form>
    <el-table
      :data="dataList"
      ...
      <el-table-column prop="type" label="状态" :show-overflow-tooltip='true' width="80">
        <template slot-scope="scope">
      ...
      <el-table-column
        ...
        header-align="center"
        align="center"
        label="备注" :show-overflow-tooltip='true' width="120">
      </el-table-column>
      <el-table-column
        fixed="right"
        header-align="center"
        ...
        label="操作">
        <template slot-scope="scope">
          <el-button v-if="isAuth('generator:pfacfix:update')"  :disabled="scope.row.status != 2" type="success" size="small" @click="addOrUpdateHandle(scope.row.id)">完成维修</el-button>
...
<script>
  import AddOrUpdate from './pfacfix-add-or-update'
  export default {
    data () {
      return {
        dataForm: {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/generator/pfacfix/list'),
          method: 'get',
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/generator/pfacfix/delete'),
            method: 'post',
